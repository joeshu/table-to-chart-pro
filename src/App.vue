...
async function handleExport(options:ExportOptions){try{if(workspaceError.value)return showNotice(workspaceError.value,true);if(options.batch&&options.format!=='csv'){const count=state.table.value.headers.length-1,totalPixels=count*options.width*options.height*options.scale*options.scale;if(options.format!=='svg'&&totalPixels>120_000_000)return showNotice(`批量任务过大（${count} 张），请降低尺寸、倍率或列数`,true);const files:{name:string;contents:string}[]=[];for(let index=1;index<state.table.value.headers.length;index++){const header=state.table.value.headers[index],table={headers:[state.table.value.headers[0],header],columnIds:[state.table.value.columnIds?.[0]??'category',state.table.value.columnIds?.[index]??String(index)],rows:state.table.value.rows.map(row=>[row[0],row[index]])},settings={...state.settings,title:`${state.settings.title} · ${header}`,type:['scatter','bubble','heatmap','combo'].includes(state.settings.type)?'bar' as const:state.settings.type};const extension=options.format,contents=extension==='svg'?await renderChartSvg(table,settings,options):await renderChartImage(table,settings,options);files.push({name:`${safeName(options.fileName)}-${safeName(header)}.${extension}`,contents:extension==='pdf'?createPdf(contents,options.width,options.height):contents});showNotice(`正在生成 ${index}/${state.table.value.headers.length-1}`);}if(await saveBatchFiles(files,options.format)){showExport.value=false;showNotice(`已批量导出 ${files.length} 张图表`);}return;}let contents:string,extension:ExportOptions['format']=options.format;if(options.format==='csv'){contents=createCsv(state.table.value);}else if(options.format==='svg'){contents=await renderChartSvg(state.table.value,state.settings,options);}else{const image=await renderChartImage(state.table.value,state.settings,options);contents=options.format==='pdf'?createPdf(image,options.width,options.height):image;}const ok=await saveExportFile(contents,`${options.fileName}.${extension}`,extension);if(ok){showExport.value=false;showNotice(`${extension.toUpperCase()} 已导出`);}}catch(error){showNotice(`导出失败：${message(error)}`,true);}}
...
async function handleCopySvg(){try{if(workspaceError.value)return showNotice(workspaceError.value,true);const svg=await renderChartSvg(state.table.value,state.settings,{format:'svg',width:1600,height:900,scale:1,background:'theme',fileName:projectName.value});await copySvg(svg);showNotice('SVG 已复制，可粘贴到 Figma、PPT 或 Illustrator');}catch(error){showNotice(`复制 SVG 失败：${message(error)}`,true);}}
