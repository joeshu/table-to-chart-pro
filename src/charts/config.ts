...
export function buildChartConfig(table: DataTable, settings: ChartSettings) {
  ...
  if(settings.type==='combo'){
    ...
    for(const [index,column] of orderedColumns.entries())if(!barColumns.has(column)){const trend=createTrendDataset(column,numeric(column),datasets[index],settings);if(trend)datasets.push(trend);}
...
function createTrendDataset(column:number,values:number[],base:any,settings:ChartSettings){
  const kind=settings.seriesTrendlines?.[String(column)]??'none';if(kind==='none'||values.length<2)return null;
  let data:(number|null)[]=[],equation='',r2:number|null=null;
  if(kind==='movingAverage'){const period=Math.max(2,settings.seriesMovingAveragePeriods?.[String(column)]??3);data=values.map((_,index)=>index<period-1?null:values.slice(index-period+1,index+1).reduce((sum,value)=>sum+value,0)/period);equation=`移动平均(${period})`;}
