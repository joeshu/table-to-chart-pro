  ...
  afterDatasetsDraw(chart: any, _args: unknown, options: any) {
    ...
    chart.data.datasets.forEach((dataset: any,index: number)=>{
      ...
      meta.data.forEach((element:any,dataIndex:number)=>{
        ...
        if(typeof value!=='number')return; const point=element.tooltipPosition(),position=options.position??'auto',y=position==='center'?point.y:(value>=0?(element.y??point.y)-5:(element.y??point.y)+14); const formatter=dataset.valueFormatter??options.formatter;ctx.textBaseline=position==='center'?'middle':'bottom'; ctx.fillText(formatter(value),point.x,y);
      });
    }); ctx.restore();
  },
};
...
export function buildChartConfig(table: DataTable, settings: ChartSettings) {
  ...
  if(settings.type==='combo'){
    const barColumns=new Set((settings.comboBarColumns??[1]).filter(index=>index>=1&&index<table.headers.length));
    if(!barColumns.size)barColumns.add(1);
    const allIndexes=table.headers.slice(1).map((_,index)=>index+1),hidden=new Set(settings.hiddenColumns??[]),visibleIndexes=allIndexes.filter(index=>!hidden.has(index));
    const rightSet=settings.comboRightAxisColumns===null?new Set(visibleIndexes.filter(index=>!barColumns.has(index))):new Set(settings.comboRightAxisColumns.filter(index=>visibleIndexes.includes(index)));
    ...
    const rightFormatter=(value:number)=>rightFormat==='percent'?`${(value*100).toFixed(Math.max(1,settings.decimals))}%`:formatChartValue(value,rightFormat,settings.decimals),percentScale=rightFormat==='percent'?0.01:1;
    const datasets=table.headers.slice(1).map((header,index)=>{const column=index+1,isBar=barColumns.has(column),color=palette.colors[index%palette.colors.length];return {type:isBar?'bar':'line',label:header,data:numeric(column),hidden:hidden.has(column),valueFormatter:columnFormatter(column),backgroundColor:isBar?color:color+'22',borderColor:color,borderWidth:isBar?settings.barBorderWidth:settings.lineWidth,borderRadius:isBar?settings.barRadius:0,categoryPercentage:settings.barCategoryPercentage,barPercentage:settings.barPercentage,borderDash:!isBar&&settings.lineDash?[8,5]:[],stepped:!isBar&&settings.lineStepped,tension:isBar?0:(settings.smooth?.34:0),pointRadius:isBar?0:settings.pointRadius,pointHoverRadius:isBar?0:settings.pointRadius+2,spanGaps:!isBar&&settings.connectGaps,stack:isBar&&settings.stacked?'main':undefined,yAxisID:rightSet.has(column)?'y1':'y',fill:!isBar&&settings.areaFill};});
    const scales:any=axes(palette,settings,leftPercent?(value:number)=>`${(value*100).toFixed(Math.max(1,settings.decimals))}%`:formatter);scales.y.display=leftIndexes.length>0;if(leftPercent){scales.y.min=settings.yMin===null?0:settings.yMin*.01;scales.y.max=settings.yMax===null?1:settings.yMax*.01;scales.y.ticks.stepSize=settings.yStep===null?undefined:settings.yStep*.01;}if(!settings.yAxisTitle&&leftIndexes.length===1){const leftIndex=leftIndexes[0];scales.y.title={display:true,text:table.headers[leftIndex],color:palette.text};}scales.y1={display:rightIndexes.length>0,position:'right',beginAtZero:settings.y2BeginAtZero&&settings.y2Min===null, min:settings.y2Min===null?(rightFormat==='percent'?0:undefined):settings.y2Min*percentScale,max:settings.y2Max===null?(rightFormat==='percent'?1:undefined):settings.y2Max*percentScale,title:{display:Boolean(settings.rightAxisTitle||rightIndexes.length===1),text:settings.rightAxisTitle||(rightIndexes.length===1?table.headers[rightIndexes[0]]:''),color:palette.text},border:{display:settings.showAxisBorder,color:palette.grid},grid:{display:false},ticks:{stepSize:settings.y2Step===null?undefined:settings.y2Step*percentScale,color:palette.text,callback:(value:any)=>rightFormatter(Number(value))}};
  ...
  const forceArea=settings.type==='area',rawSeries=table.headers.slice(1).map((_,index)=>numeric(index+1)),positiveTotals=table.rows.map((_,rowIndex)=>rawSeries.reduce((sum,series)=>sum+Math.max(0,series[rowIndex]),0)),negativeTotals=table.rows.map((_,rowIndex)=>rawSeries.reduce((sum,series)=>sum+Math.abs(Math.min(0,series[rowIndex])),0)),hasNegative=rawSeries.some(series=>series.some(value=>value<0)),visibleColumns=table.headers.slice(1).map((_,index)=>index+1).filter(index=>!(settings.hiddenColumns??[]).includes(index)),allPercent=!settings.percentageStacked&&visibleColumns.length>0&&visibleColumns.every(isPercentColumn);
  const datasets=table.headers.slice(1).map((header,index)=>({label:header,valueFormatter:columnFormatter(index+1),data:settings.percentageStacked?rawSeries[index].map((value,rowIndex)=>{const total=value<0?negativeTotals[rowIndex]:positiveTotals[rowIndex];return total?value/total*100:0;}):(type==='line'?numericNullable(index+1):rawSeries[index]),backgroundColor:type==='line'||type==='radar'?(forceArea||settings.areaFill||type==='radar'?palette.colors[index%palette.colors.length]+'33':'transparent'):palette.colors[index%palette.colors.length],borderColor:palette.colors[index%palette.colors.length],hidden:(settings.hiddenColumns??[]).includes(index+1),borderWidth:type==='bar'?settings.barBorderWidth:settings.lineWidth,borderRadius:type==='bar'?settings.barRadius:0,categoryPercentage:settings.barCategoryPercentage,barPercentage:settings.barPercentage,borderDash:type==='line'&&settings.lineDash?[8,5]:[],stepped:type==='line'&&settings.lineStepped,tension:settings.smooth?.36:0,fill:type==='radar'||forceArea||(type==='line'&&settings.areaFill),spanGaps:settings.connectGaps,stack:settings.stacked?'main':undefined,pointRadius:type==='line'?settings.pointRadius:undefined,pointHoverRadius:type==='line'?settings.pointRadius+2:undefined}));
  const axisFormatter=allPercent?(value:number)=>`${(value*100).toFixed(Math.max(1,settings.decimals))}%`:formatter;
  ...
  if(allPercent&&type!=='radar'){options.scales.y.min=settings.yMin===null?0:settings.yMin*.01;options.scales.y.max=settings.yMax===null?undefined:settings.yMax*.01;options.scales.y.ticks.stepSize=settings.yStep===null?undefined:settings.yStep*.01;}
...
function axes(palette:{text:string;grid:string},settings:ChartSettings,formatter:(value:number)=>string,percentageHasNegative=false){
  const category={stacked:settings.stacked,title:{display:Boolean(settings.xAxisTitle),text:settings.xAxisTitle,color:palette.text},border:{display:settings.showAxisBorder,color:palette.grid},grid:{display:false},ticks:{color:palette.text,minRotation:settings.xLabelRotation,maxRotation:settings.xLabelRotation,maxTicksLimit:settings.xMaxTicks,autoSkip:true}};
  const value={stacked:settings.stacked,beginAtZero:settings.yBeginAtZero&&(settings.yMin===null||settings.yMin===undefined),min:settings.percentageStacked?(percentageHasNegative?-100:0):(settings.yMin??undefined),max:settings.percentageStacked?100:(settings.yMax??undefined),title:{display:Boolean(settings.yAxisTitle),text:settings.yAxisTitle,color:palette.text},border:{display:settings.showAxisBorder,color:palette.grid},grid:{display:settings.showGrid,color:palette.grid,lineWidth:settings.gridLineWidth},ticks:{stepSize:settings.yStep??undefined,color:palette.text,callback:(value:any)=>settings.percentageStacked?`${Number(value).toFixed(0)}%`:formatter(Number(value))}};
