...
export function buildChartConfig(table: DataTable, settings: ChartSettings) {
  ...
  if(settings.type==='combo'){
    ...
    const rightIndexes=table.headers.slice(1).map((_,index)=>index+1).filter(index=>!barColumns.has(index));
    const rightPercent=rightIndexes.length>0&&rightIndexes.every(index=>isPercentColumn(index)), leftPercent=[...barColumns].every(index=>isPercentColumn(index)), rightFormat=settings.rightAxisFormat&&settings.rightAxisFormat!=='auto'?settings.rightAxisFormat:(rightPercent?'percent':settings.numberFormat);
    ...
    const datasets=table.headers.slice(1).map((header,index)=>{const column=index+1,isBar=barColumns.has(column);return {type:isBar?'bar':'line',label:header,data:numeric(column),valueFormatter:columnFormatter(column),backgroundColor:isBar?palette.colors[index%palette.colors.length]:palette.colors[index%palette.colors.length]+'22',borderColor:palette.colors[index%palette.colors.length],borderRadius:isBar?5:0,tension:isBar?0:.34,pointRadius:isBar?0:4,yAxisID:isBar?'y':'y1',fill:false};});
    const scales:any=axes(palette,settings,leftPercent?(value:number)=>`${(value*100).toFixed(Math.max(1,settings.decimals))}%`:formatter);if(leftPercent){scales.y.min=settings.yMin===null?0:settings.yMin*.01;scales.y.max=settings.yMax===null?1:settings.yMax*.01;scales.y.ticks.stepSize=settings.yStep===null?undefined:settings.yStep*.01;}if(!settings.yAxisTitle&&barColumns.size===1){const leftIndex=[...barColumns][0];scales.y.title={display:true,text:table.headers[leftIndex],color:palette.text};}scales.y1={display:rightIndexes.length>0,position:'right',beginAtZero:settings.y2BeginAtZero&&settings.y2Min===null, min:settings.y2Min===null?(rightFormat==='percent'?0:undefined):settings.y2Min*percentScale,max:settings.y2Max===null?(rightFormat==='percent'?1:undefined):settings.y2Max*percentScale,title:{display:Boolean(settings.rightAxisTitle||rightIndexes.length===1),text:settings.rightAxisTitle||(rightIndexes.length===1?table.headers[rightIndexes[0]]:''),color:palette.text},grid:{display:false},ticks:{stepSize:settings.y2Step===null?undefined:settings.y2Step*percentScale,color:palette.text,callback:(value:any)=>rightFormatter(Number(value))}};
