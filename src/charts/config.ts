  ...
  afterDatasetsDraw(chart: any, _args: unknown, options: any) {
    ...
    chart.data.datasets.forEach((dataset: any,index: number)=>{
      ...
      meta.data.forEach((element:any,dataIndex:number)=>{
        ...
        if(typeof value!=='number')return; const point=element.tooltipPosition(),position=options.position??'auto',y=position==='center'?point.y:(value>=0?(element.y??point.y)-5:(element.y??point.y)+14); const formatter=dataset.valueFormatter??options.formatter,text=formatter(value),width=ctx.measureText(text).width+8,height=fontSize+6;ctx.textBaseline='middle';if(options.background){ctx.fillStyle=options.background;ctx.fillRect(point.x-width/2,y-height/2,width,height);}if(options.borderColor){ctx.strokeStyle=options.borderColor;ctx.strokeRect(point.x-width/2,y-height/2,width,height);}ctx.fillStyle=options.color;ctx.fillText(text,point.x,y);
      });
    }); ctx.restore();
  },
};
...
export function buildChartConfig(table: DataTable, settings: ChartSettings) {
  ...
  const sourceColumns=table.headers.slice(1).map((_,index)=>index+1),orderedColumns=[...(settings.seriesOrder??[]).filter(column=>sourceColumns.includes(column)),...sourceColumns.filter(column=>!(settings.seriesOrder??[]).includes(column))];
  const seriesName=(column:number)=>settings.seriesNames?.[String(column)]||table.headers[column],seriesOpacity=(column:number)=>settings.seriesOpacity?.[String(column)]??1;
  ...
  if(settings.type==='combo'){
    ...
    const allIndexes=orderedColumns,hidden=new Set(settings.hiddenColumns??[]),visibleIndexes=allIndexes.filter(index=>!hidden.has(index));
    ...
    const datasets=orderedColumns.map(column=>{const index=column-1,header=seriesName(column),isBar=barColumns.has(column),color=palette.colors[index%palette.colors.length],opacity=seriesOpacity(column),lineWidth=settings.seriesLineWidths?.[String(column)]??settings.lineWidth,pointSize=settings.seriesPointRadii?.[String(column)]??settings.pointRadius,dashed=settings.seriesDashed?.[String(column)]??settings.lineDash;return {type:isBar?'bar':'line',label:header,data:numeric(column),hidden:hidden.has(column),valueFormatter:columnFormatter(column),backgroundColor:isBar?alphaColor(color,opacity):alphaColor(color,.14*opacity),borderColor:alphaColor(color,opacity),borderWidth:isBar?settings.barBorderWidth:lineWidth,borderRadius:isBar?settings.barRadius:0,categoryPercentage:settings.barCategoryPercentage,barPercentage:settings.barPercentage,borderDash:!isBar&&dashed?[8,5]:[],stepped:!isBar&&settings.lineStepped,tension:isBar?0:(settings.smooth?.34:0),pointRadius:isBar?0:pointSize,pointHoverRadius:isBar?0:pointSize+2,spanGaps:!isBar&&settings.connectGaps,stack:isBar&&settings.stacked?'main':undefined,yAxisID:rightSet.has(column)?'y1':'y',fill:!isBar&&settings.areaFill};});
    const scales:any=axes(palette,settings,leftPercent?(value:number)=>`${(value*100).toFixed(Math.max(1,settings.yAxisDecimals??settings.decimals))}%`:(value:number)=>formatChartValue(value,settings.numberFormat,settings.yAxisDecimals??settings.decimals));scales.y.display=leftIndexes.length>0;if(leftPercent){scales.y.min=settings.yMin===null?0:settings.yMin*.01;scales.y.max=settings.yMax===null?1:settings.yMax*.01;scales.y.ticks.stepSize=settings.yStep===null?undefined:settings.yStep*.01;}if(!settings.yAxisTitle&&leftIndexes.length===1){const leftIndex=leftIndexes[0];scales.y.title={display:true,text:seriesName(leftIndex),color:settings.yAxisColor||palette.text,font:{size:settings.yAxisFontSize}};}scales.y1={display:rightIndexes.length>0,position:'right',beginAtZero:settings.y2BeginAtZero&&settings.y2Min===null, min:settings.y2Min===null?(rightFormat==='percent'?0:undefined):settings.y2Min*percentScale,max:settings.y2Max===null?(rightFormat==='percent'?1:undefined):settings.y2Max*percentScale,title:{display:Boolean(settings.rightAxisTitle||rightIndexes.length===1),text:settings.rightAxisTitle||(rightIndexes.length===1?seriesName(rightIndexes[0]):''),color:settings.y2AxisColor||palette.text,font:{size:settings.y2AxisFontSize}},border:{display:settings.showAxisBorder,color:palette.grid},grid:{display:false},ticks:{stepSize:settings.y2Step===null?undefined:settings.y2Step*percentScale,color:settings.y2AxisColor||palette.text,font:{size:settings.y2AxisFontSize},callback:(value:any)=>rightFormatter(Number(value))}};
