...
const mobileQuery=window.matchMedia('(max-width: 680px)'),rowHeight=ref(mobileQuery.matches?45:35),buffer=8;
function syncRowHeight(){rowHeight.value=mobileQuery.matches?45:35;}
onMounted(()=>mobileQuery.addEventListener('change',syncRowHeight));onUnmounted(()=>mobileQuery.removeEventListener('change',syncRowHeight));
const errors = computed(() => props.issues.filter(issue => issue.level === 'error'));
const invalidCells=computed(()=>new Set(errors.value.map(issue=>`${issue.row}:${issue.col}`)));
const virtualized = computed(() => props.table.rows.length > 120);
const start = computed(() => virtualized.value ? Math.max(0, Math.floor(scrollTop.value / rowHeight.value) - buffer) : 0);
const end = computed(() => virtualized.value ? Math.min(props.table.rows.length, start.value + Math.ceil(520 / rowHeight.value) + buffer * 2) : props.table.rows.length);
...
<template>
  <aside class="data-panel panel-shell">
    ...
    <template v-else>
      <div class="table-tools"><button class="import-button" @click="emit('import',encoding)">导入文件</button><select v-model="encoding" class="encoding-select" aria-label="导入文本编码"><option value="utf-8">UTF-8</option><option value="gbk">GBK</option><option value="gb18030">GB18030</option></select><button @click="emit('addRow')">＋ 行</button><button @click="emit('addColumn')">＋ 列</button><button class="danger-text" :disabled="!selected.size" @click="remove">删除 {{ selected.size || '' }}</button><span class="virtual-hint">{{table.rows.length > 120 ? `已启用虚拟渲染 · ${table.rows.length} 行` : ''}}</span></div>
      <div class="workspace-table-wrap" @scroll="onScroll"><table class="workspace-table"><thead><tr><th class="select-col"></th><th class="index-col">#</th><th v-for="(header,col) in table.headers" :key="col"><div class="column-head"><input :value="header" @change="emit('header',col,($event.target as HTMLInputElement).value)"><button v-if="table.headers.length>2" title="删除列" @click="emit('deleteColumn',col)">×</button></div></th></tr></thead><tbody><tr v-if="table.rows.length>120" class="virtual-spacer"><td :colspan="table.headers.length+2" :style="{height:`${start*rowHeight}px`}"></td></tr><tr v-for="item in visibleRows" :key="item.index" :class="{ selected:selected.has(item.index) }"><td class="select-col"><input type="checkbox" :checked="selected.has(item.index)" @change="toggle(item.index,($event.target as HTMLInputElement).checked)"></td><td class="index-col">{{ item.index+1 }}</td><td v-for="(cell,colIndex) in item.row" :key="colIndex"><input :value="cell" :class="{ invalid: invalidCells.has(`${item.index}:${colIndex}`) }" @change="emit('cell',item.index,colIndex,($event.target as HTMLInputElement).value)"></td></tr><tr v-if="table.rows.length>120" class="virtual-spacer"><td :colspan="table.headers.length+2" :style="{height:`${(table.rows.length-end)*rowHeight}px`}"></td></tr></tbody></table></div>
    </template>
  </aside>
</template>
